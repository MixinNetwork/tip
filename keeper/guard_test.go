...
func TestGuard(t *testing.T) {
	...
	assert.Nil(err)
	assert.Equal(1, counter)
	assert.Equal(oas, crypto.PublicKeyBytes(userPub))
	...
	assert.Nil(err)
	assert.Nil(res.Watcher)
	assert.Equal(SecretLimitQuota-6, res.Available)
	...
	assert.Nil(err)
	assert.Nil(res.Watcher)
	assert.Equal(SecretLimitQuota-7, res.Available)
	oas, _, counter, err = bs.Watch(watcherSeed)
	assert.Nil(err)
	assert.Equal(3, counter)
	assert.Equal(oas, crypto.PublicKeyBytes(userPub))
	...
	assert.Nil(err)
	assert.Nil(res.Watcher)
	assert.Equal(SecretLimitQuota-8, res.Available)
